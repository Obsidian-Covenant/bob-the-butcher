...
# define NB_SPACE    1  /* treat this as "indent levels", not chars */

/* g_space is an indent *level*. Don't let it underflow. */
# define UP_SPACE_LEVEL      (g_space += NB_SPACE)
# define DOWN_SPACE_LEVEL    do {                      \
        if (g_space >= NB_SPACE)                       \
            g_space -= NB_SPACE;                       \
        else                                           \
            g_space = 0;                               \
...
/* Write helper: still discards result, just not to stdin. */
# define SAFE_WRITE(fd, buf, len)                      \
    do {                                               \
        ssize_t _w = write((fd), (buf), (len));        \
        (void)_w;                                      \
    } while (0)

/* Send debug indentation to stderr (fd 2) */
#define TAB_WRITE                                      \
    do {                                               \
        int i = 0;                                     \
        char c = '\t';                                 \
        /* print exactly g_space tabs, not g_space+1 */\
        while (i++ < (int)g_space)                     \
            SAFE_WRITE(2, &c, 1);                      \
    } while (0)
