...
void	newjob(int c, char **v, struct s_admin_opt *opt)
{
	...
	TAILQ_FOREACH(pwd, &l_pwd, next)
	{
		...
		net_crypt(net, (unsigned char *)&tmppwd, sizeof(struct s_pwd));
...
void    getinf(int c, char **v, struct s_admin_opt *opt)
{
	...
	time_t t_date_start = (time_t)date_start;
	timestruct = gmtime(&t_date_start);
	...
	printf("Keys processed : %" PRIu64 " in %d secs (%" PRIu64 " %ck/s) (Approx to %s)\n",
