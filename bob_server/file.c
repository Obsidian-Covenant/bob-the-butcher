...
int     load_file (const char *fl)
{
	...
	while(1)
	{
		...
		{
			case 1:
				if (sscanf(buf, "%d:%[^:]:%ld:%hhu:%hhd:%d:%" SCNu64,
				           &(tmpjob.id),
				           tmp1,
				           &(tmpjob.date_start),
				           &(tmpjob.status),
			...
			case 3:
				if(!curjob)
					break;
				if (sscanf(buf, "%" SCNu64 ":%" SCNu64 ":%d", &start, &end, &istatus) == 3)
				{
				    status = (enum status_space)istatus;
				    if (status != DONE)
...
void    save_file (void)
{
	...
	{
		fprintf(file, "job\n");
		fprintf(file, "%d:%s:%u:%d:%d:%d:%" PRIu64 "\n", 
		        job->id,
		        job->name,
