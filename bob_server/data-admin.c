...
int admin_status(int fd, struct s_array_client *client, struct s_rw * rw, struct s_network * net)
{
	...
	TAILQ_FOREACH(job, hjobs, next)
	{
		...
		str_append_netstring(
		    net->str,
		    (unsigned char *)job->name,
		    strlen(job->name)
		);
	}
...
int     admin_add_job(int fd, struct s_array_client *client, struct s_rw * rw, struct s_network * net)
{
	...
	net_crypt(net, (unsigned char *)&job_id, sizeof(uint32_t));
	ssize_t w = write(fd, &job_id, sizeof(uint32_t));
	if (w != (ssize_t)sizeof(uint32_t)) {
	    fprintf(stderr, "admin_add_job: failed to write job_id "
	                    "(wrote %zd of %zu bytes): %s\n",
	            w, sizeof(uint32_t), strerror(errno));
	    /* Depending on protocol, you might want to return an error */
	}
...
int     admin_add_pwd(int fd, struct s_array_client *client, struct s_rw *rw, struct s_network * net)
{
	...
	for (i = nb_pwd; i > 0; i--)
	{
		...
		/* FIXME fix this */
		nsize = net_read_buf(fd, (char *)&pwd, sizeof(struct s_pwd), net);
		if (nsize < 0)
			break;
...
int admin_getinf(int fd, struct s_array_client *client, struct s_rw * rw, struct s_network * net)
{
    ...
    str_append_netstring(str, (unsigned char *)job->name, strlen(job->name));
    ...
    printf("processed space = %" PRIu64 "\n", pspace.done);
    str_append_int64(str, pspace.done);
    str_append_int32(str, time(NULL) - job->date_start);

    TAILQ_FOREACH(passwd, job->passwd, next) {
        if (passwd->cleartext) {
            str_append_netstring(str, (unsigned char *)passwd->username,
                                 strlen(passwd->username));
            str_append_netstring(str, (unsigned char *)passwd->password,
                                 strlen(passwd->password));
            str_append_netstring(str, (unsigned char *)passwd->cleartext,
                                 strlen(passwd->cleartext));
        }
    }

    /* Now send: length, seed, encrypted payload */
    psize = htonl(STRLEN(str));

    if (safe_write(fd, &psize, sizeof(psize)) < 0) {
        fprintf(stderr, "admin_getinf: failed to write packet size\n");
        return NET_EXIT;  /* use your project's error code */
    }

    if (safe_write(fd, net->seed, CRYPTO_SEED_SIZE) < 0) {
        fprintf(stderr, "admin_getinf: failed to write crypto seed\n");
        return NET_EXIT;
    }

    net_crypt(net, STRING(str), STRLEN(str));

    if (safe_write(fd, STRING(str), STRLEN(str)) < 0) {
#ifdef DEBUG
        TAB_WRITE;
