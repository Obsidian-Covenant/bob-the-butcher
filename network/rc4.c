...
void net_crypto_init_state(struct s_network * net)
{
	...
	l = strlen((const char *)key) + CRYPTO_SEED_SIZE;
	buffer = malloc(l + 1);
	if(!buffer)
	{
#ifdef DEBUG
	...
	strcpy((char *)buffer + CRYPTO_SEED_SIZE, (const char *)key);
...
int net_crypto_init(struct s_network * net)
{
	...
	size_t nread = fread(net->seed, CRYPTO_SEED_SIZE, 1, random);
	if (nread != 1) {
	#ifdef DEBUG
	    TAB_WRITE;
	    printf(" cannot read random seed\n");
	#endif
	    fclose(random);
