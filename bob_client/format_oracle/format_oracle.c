...
static void oracle_set_username(char *salt) {
	salt_offset = 0;

	memset(salt_iv, 0, 8);
	salt_length = to_unicode_b((uint16_t *)cur_salt, (unsigned char *)salt);
	//faut gagner du temps
	if(salt_length > 4)
	{
		salt_offset = (salt_length & (~3))*2;
		my_des_ncbc_encrypt(cur_salt,
                           salt_offset,
                           &desschedule1,
                           (DES_cblock *)salt_iv);
	}
...
static void oracle_set_key(char *key, int index) {
	key_length = to_unicode_b((uint16_t *)(cur_salt + salt_length*2),
                             (unsigned char *)key);
}
...
static unsigned int oracle_crypt_all(int count)
{
	unsigned int l;
	...
	my_des_ncbc_encrypt(cur_salt + salt_offset,
                        l - salt_offset,
                        &desschedule1,
                        (DES_cblock *)crypt_key);
