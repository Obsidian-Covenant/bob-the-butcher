...
static void stop_timer_handler(int sig)
{
    (void)sig;     /* unused */
    stop_timer();
}

static void abort_job_handler(int sig)
{
    (void)sig;     /* unused */
    abort_job();
}
...
{
    signal(SIGALRM, stop_timer_handler);
    signal(SIGTERM, abort_job_handler);
    signal(SIGINT, abort_job_handler);
}
