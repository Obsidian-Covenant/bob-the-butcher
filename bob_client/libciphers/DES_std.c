...
ARCH_WORD * DES_raw_get_salt(char *ciphertext)
{
	...
	out = malloc(sizeof(ARCH_WORD));
	
	if (ciphertext[13]) *out = DES_atoi64[ARCH_INDEX(ciphertext[5])] |
	...
	else *out = DES_atoi64[ARCH_INDEX(ciphertext[0])] |
...
ARCH_WORD * DES_std_get_salt(char *ciphertext)
{
    ...
    *salt = (ARCH_WORD)DES_24_TO_32((unsigned int)*salt);
    *salt = (ARCH_WORD)DES_DO_SIZE_FIX(*salt);
    return salt;
}
...
ARCH_WORD *DES_std_get_binary(char *ciphertext)
{
	...
	out = malloc(4*sizeof(ARCH_WORD));
	memset(out, 0, 4*sizeof(ARCH_WORD));
