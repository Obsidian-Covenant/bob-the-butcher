...
#define MAX_PWD_SCAN_WIDTH  63  /* MAX_PWD_LEN - 1, but as a plain number */
...
new_username:
	...
	if(opt.debug)
		printf("Start working %s with algo %s (%" PRIu64 ")", password + pos, fmt->params.algname, cur);
...
int john_crack(struct s_format * fmt)
{
    ...
    pjohn = popen((char *)cmdline, "r");
    if (!pjohn)
        goto john_error;

	while (fgets((char *)output, sizeof(output), pjohn) != NULL)
	{
	    char username[MAX_PWD_LEN];
	    char password[MAX_PWD_LEN];

	    /* Parse "username password" from John’s output line */
	    if (sscanf((char *)output,
	               "%" STR(MAX_PWD_SCAN_WIDTH) "s %" STR(MAX_PWD_SCAN_WIDTH) "s",
	               username, password) == 2)
	    {
	        client_password_found((unsigned char *)username, NULL,
